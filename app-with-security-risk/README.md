...
OpenTelemetry/Splunk instrumentation, Cisco AI Defense Gateway integration,
and [example screenshots](#examples-including-traces-with-evaluation-in-splunk-observability-cloud)
...
agent to summarize the policy status. This `app-with-security-risk` variant
wraps LLM outputs with `PoisonedChatWrapper` to inject simulated quality issues
and security/PII risks into trace-captured agent responses. Without a
`quality_issue_scenario` request field, the policy agent keeps the original
default policy-narrative quality issue. Scenario payloads can target other
LLM-backed agents. The deterministic policy result still comes from
`evaluate_policy(...)`.
...
### Cisco AI Defense Gateway Mode

This variant is configured for Cisco AI Defense Gateway mode in Kubernetes. The
gateway URL is supplied as `AZURE_OPENAI_ENDPOINT` from `ai-defense-secret`, and
the Azure OpenAI API key is still supplied as `AZURE_OPENAI_API_KEY` from
`azure-openai-api`. This mirrors the travel-agent workshop pattern: the app code
continues to use `AzureChatOpenAI`, while traffic is routed through the AI
Defense gateway.

Security scenario payloads use `security_risk_scenario` to inject fake/test PII
or PCI content into selected LLM-backed agent output. The final JSON response is
still redacted and deterministic; the simulated risk is intended to appear in
trace-captured model content so AI Defense can report a security event. In
Splunk Observability Cloud, look for security risks on the AI overview, AI agent,
and AI trace views. When Cisco AI Defense records an event, the related span can
include `gen_ai.security.event_id`.

If your AI Defense policy is configured to block instead of monitor, the request
may fail or return differently at runtime; that block is still a valid security
event for the demo.
...
```bash
cd /Users/zratko/Downloads/ZR-agentic-ai-demo/agentic-ai-homeloan/app-with-security-risk
...
Create the Cisco AI Defense gateway secret in the `home-loan-agent` namespace:

```bash
kubectl create secret generic ai-defense-secret \
  -n home-loan-agent \
  --from-literal=ai-defense-gateway-url='GATEWAY_URL_FROM_AI_DEFENCE' \
  --dry-run=client -o yaml | kubectl apply -f -
```

Create the Azure OpenAI API key secret in the `home-loan-agent` namespace:
...
`secret "ai-defense-secret"`, `secret "azure-openai-api"`, or
...
curl http://home-loan-broker.localhost/home-loan/assess \
  -H "Content-Type: application/json" \
  -d @sample_payloads/security_pii_identity_leak.json

curl http://home-loan-broker.localhost/home-loan/assess \
  -H "Content-Type: application/json" \
  -d @sample_payloads/security_pci_fee_card_leak.json
```
...
- `sample_payloads/security_pii_identity_leak.json`: targets A2 `kyc_aml` with a fake/test identity-data disclosure.
- `sample_payloads/security_pci_fee_card_leak.json`: targets A5 `risk_compliance` with a fake/test card-data disclosure.
...
### Security Risk Scenario Payloads

These payloads include `security_risk_scenario`. That field is used only to
inject controlled fake/test PII or PCI content into the selected LLM-backed
trace span; it does not change deterministic lending calculations, reason
codes, or final outcomes.

- `sample_payloads/security_pii_identity_leak.json`: targets A2 `kyc_aml` with fake identity identifiers.
- `sample_payloads/security_pci_fee_card_leak.json`: targets A5 `risk_compliance` with a standard test Visa number.

`run_assessments.sh` interleaves these security payloads with the existing
quality and normal workflow examples. You can override the defaults without
editing the file:

```bash
BASE_URL=http://localhost:8080/home-loan/assess \
DURATION_HOURS=1 \
SLEEP_MIN_SECONDS=10 \
SLEEP_JITTER_SECONDS=5 \
bash run_assessments.sh
```
...
```bash
HOME_LOAN_DETERMINISTIC_ONLY=true python3 -m unittest discover -s tests
```
...
![Example: AI apps overview](/app-with-security-risk/sample_payloads/screenshots/ai-apps-overview.png)

![Example: AI Agents list](/app-with-security-risk/sample_payloads/screenshots/ai-agents-all.png)

![Example: AI agent view](/app-with-security-risk/sample_payloads/screenshots/ai-agent-view.png)

![Example: AI traces](/app-with-security-risk/sample_payloads/screenshots/ai-traces-data.png)

![Example: AI agent flow](/app-with-security-risk/sample_payloads/screenshots/ai-agent-flow.png)

![Example: The applicant is careless and financially incompetent](/app-with-security-risk/sample_payloads/screenshots/quality-toxic.png)

![Example: Eval explanation](/app-with-security-risk/sample_payloads/screenshots/quality-toxic-eval.png)
