...
SECURITY_SCENARIO_TARGETS = {
    "pii_identity_leak": ("kyc_aml", "pii"),
    "pci_fee_card_leak": ("risk_compliance", "pci"),
}
...
class HomeLoanDeterministicTests(unittest.TestCase):
    ...
    def test_security_risk_scenarios_route_to_target_agents(self):
        all_target_agents = {
            "broker_orchestrator",
            "conversation_intake",
            "kyc_aml",
        ...
        for scenario, (target_agent, category) in SECURITY_SCENARIO_TARGETS.items():
            with self.subTest(scenario=scenario):
                state = {
                    "security_risk_scenario": main._normalise_security_risk_scenario(
                        scenario
                    )
                }
                issue = main._security_risk_for_agent(state, target_agent)

                self.assertIsNotNone(issue)
                self.assertEqual(issue["category"], category)
                self.assertIn("Security risk scenario:", issue["snippet"])

                for other_agent in all_target_agents - {target_agent}:
                    self.assertIsNone(main._security_risk_for_agent(state, other_agent))

        self.assertIsNone(main._normalise_security_risk_scenario("unknown_scenario"))

    def test_quality_and_security_scenarios_can_coexist_on_one_agent(self):
        state = {
        ...
        snippet = main._observability_issue_snippet_for_agent(state, "risk_compliance")
        metadata = main._agent_config("risk_compliance", state, "unit test")["metadata"]

        self.assertIn("Quality issue scenario:", snippet)
        self.assertIn("Security risk scenario:", snippet)
        self.assertEqual(metadata["home_loan.quality_issue.category"], "sentiment")
        self.assertEqual(metadata["home_loan.security_risk.category"], "pci")
...
class HomeLoanApiTests(unittest.TestCase):
    ...
    def test_sample_payloads_execute_expected_paths(self):
        ...
        expected_outcomes = {
            ...
            "security_pii_identity_leak.json": "APPROVE_IN_PRINCIPLE",
            "security_pci_fee_card_leak.json": "REFER",
        }
        ...
        for filename, expected_outcome in expected_outcomes.items():
            with self.subTest(filename=filename):
                ...
                self.assertNotIn("Security risk scenario:", serialized)
                self.assertNotIn("078-05-1120", serialized)
                ...
                if payload.get("security_risk_scenario"):
                    self.assertNotIn(payload["security_risk_scenario"], serialized)
