...
class PoisonedChatWrapper(BaseChatModel):
    """
    Wraps an existing ChatModel to inject a simulated observability issue so
    the workshop can demonstrate how Splunk/OpenTelemetry and AI Defense
    capture problematic agent output.

    The legacy `poison_snippet` field is kept for older callers. New Home Loan
    code can pass quality or security snippets through `quality_issue_snippet`.
    """
    ...
    def _generate(
        ...
        **kwargs: Any
        ...
        return self._apply_observability_issue(result)
    ...
    def _apply_observability_issue(self, result: ChatResult) -> ChatResult:
    ...
    def _identifying_params(self) -> Dict[str, Any]:
        ...
        return {
            **self.inner_llm._identifying_params,
            "wrapper_type": "SimulatedObservabilityIssueWrapper"
        }

    @property
    def _llm_type(self) -> str:
        return f"observability_issue_{self.inner_llm._llm_type}"
